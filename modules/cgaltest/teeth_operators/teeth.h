...
public:
    ...
    Polyhedron& get_teeth_poly() { return m_teeth_poly; }
    VertDoubleMap& get_smoothed_ray_map() { return m_vert_smooth_ray_map; }
    VertDoubleMap& get_vertex_inside_function_map() { return m_vert_inside_func_map; }
    Polyhedron& get_inside_teeth_poly() { return m_teeth_inside; }
    VertDoubleMap& get_smoothed_convex_map() { return m_inside_vert_smooth_convex_map; }
    VertDoubleMap& get_vertex_convex_function_map() { return m_inside_vert_convex_func_map; }

    int size_of_facets() { return static_cast<int>(m_teeth_poly.size_of_facets()); }
    int size_of_vertices() { return static_cast<int>(m_teeth_poly.size_of_vertices()); }
    int size_of_inside_facets() { return static_cast<int>(m_teeth_inside.size_of_facets()); }
    int size_of_inside_vertices() { return static_cast<int>(m_teeth_inside.size_of_vertices()); }
    ...
    void compute_convexity_from_sampling(double radius_ratio, double bounding_radius)
    {
        ...
        for(Vertex_handle vd: vertices(m_teeth_inside))
        {
            ...
            mean_coord = mean_coord / neighbor_points.size();
    ...
    Polyhedron& get_tooth_poly(int index) { return m_tooth[index].get_tooth_poly(); }
    VertDoubleMap& get_geodesic_distance_map(int index) { return m_tooth[index].get_geodesic_distance_map(); }
    Polyhedron& get_pad_outline_poly(int index) { return m_tooth[index].get_pad_outline_poly(); }

    HEdgeSet& get_upper_bound_set(int index) { return m_tooth[index].get_upper_bound_set(); }
    PointList& get_cut_points(int index) { return m_tooth[index].get_cut_points(); }
    Plane& get_cut_plane(int index) { return m_tooth[index].get_cut_plane(); }

    Wire& get_wire(int index) { return m_wires[index]; }
    ...
    void compute_paired_wires(int index, double distance)
    {
        ...
        // compute left and right polylines
        std::vector<PointList> right_polylines, left_polylines;
        CGAL::Polygon_mesh_processing::surface_intersection(right_halftooth, pair_plane_mesh, std::back_inserter(right_polylines));
        CGAL::Polygon_mesh_processing::surface_intersection(left_halftooth, pair_plane_mesh, std::back_inserter(left_polylines));
        if(right_polylines.size() != 1 || left_polylines.size() != 1)
        {
            std::cout << "Wrong intersection!" << std::endl;
            return;
        }
        // merge polylines
        PointList polylines;
        size_t size_right_poly = right_polylines[0].size();
        size_t size_left_poly = left_polylines[0].size();
        bool flag_left_inverse = (CGAL::squared_distance(left_polylines[0][0], right_polylines[0][0]) < CGAL::squared_distance(left_polylines[0][size_left_poly-1], right_polylines[0][0]));
        bool flag_right_inverse = (CGAL::squared_distance(left_polylines[0][0], right_polylines[0][0]) > CGAL::squared_distance(left_polylines[0][0], right_polylines[0][size_right_poly-1]));

        for(int i = 0; i < size_left_poly; i++)
        {
            if(flag_left_inverse)
                polylines.push_back(left_polylines[0][size_left_poly-i-1]);
            else
                polylines.push_back(left_polylines[0][i]);
        }

        for(int i = 0; i < size_right_poly; i++)
        {
            if(flag_right_inverse)
                polylines.push_back(right_polylines[0][size_right_poly-i-1]);
            else
                polylines.push_back(right_polylines[0][i]);
        }
    ...
    void compute_wire_shapes(int index, int discretize_step, double width)
    {
        ...
        VertVecMap vnormals;
        CGAL::Polygon_mesh_processing::compute_vertex_normals(left_halfwire, boost::make_assoc_property_map(vnormals));
        Bottom left_bottom;
        Top left_top(vnormals, width * 2.);
        CGAL::Polygon_mesh_processing::extrude_mesh(left_halfwire, extrude_leftwire, left_bottom, left_top);
        CGAL::Polygon_mesh_processing::orient_to_bound_a_volume(extrude_leftwire);

        vnormals.clear();
        CGAL::Polygon_mesh_processing::compute_vertex_normals(right_halfwire, boost::make_assoc_property_map(vnormals));
        Bottom right_bottom;
        Top right_top(vnormals, width * 2.);
        CGAL::Polygon_mesh_processing::extrude_mesh(right_halfwire, extrude_rightwire, right_bottom, right_top);
        CGAL::Polygon_mesh_processing::orient_to_bound_a_volume(extrude_rightwire);

        vnormals.clear();
        CGAL::Polygon_mesh_processing::compute_vertex_normals(middle_wire, boost::make_assoc_property_map(vnormals));
        Bottom middle_bottom;
        Top middle_top(vnormals, width * 2.);
        CGAL::Polygon_mesh_processing::extrude_mesh(middle_wire, extrude_middle, middle_bottom, middle_top);
        CGAL::Polygon_mesh_processing::orient_to_bound_a_volume(extrude_middle);
        ...
        // Wrapper
        //Polyhedron wrap_mesh;
        //CGAL::alpha_wrap_3(extrude_mesh, 0.05, 0.05 , wrap_mesh);
        //CGAL::Polygon_mesh_processing::orient(wrap_mesh);
                                            ...
                                            Polyhedron& halfwire, Point& upper_bound, Point& lower_bound)   // output
    {
        ...
        {    
            if(hedge->is_border_edge() && he_point_map.find(hedge) != he_point_map.end()) 
            {
                ...
                if(CGAL::squared_distance(p, cut_plane.projection(p)) > 1e-3)
                    bounds.push_back(p);
            }
        }
