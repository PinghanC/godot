...
inline void assemble_laplacian_matrix(Polyhedron& polymesh, ESTripleList& LTriplets, EVector& BVector, VertIntMap& vert_ind_map)
{
    for(Vertex_handle vd: vertices(polymesh))
    {
        ...
        do {
            ...
            double cot_uv = 0.5 * (u * v) / std::max(1e-7, std::sqrt(CGAL::cross_product(u, v).squared_length()));
            diag += cot_uv;
...
inline void assemble_constraint_matrix(double lambda, ESTripleList& LTriplets, EVector& BVector, IntDoubleMap& vert_constraint_map, int total_vert_num)
{
