...
Function::Function() : m_upper_init(false), m_lower_init(false), m_radius(1.), m_center(Point(0., 0., 0.)) {}
...
bool Function::compute_laplacian_based_inside(int smooth_range, double lambda, double value)
{
...
bool Function::compute_laplacian_based_convexity(double radius_ratio, int smooth_range, double lambda)
{
    if (!m_upper_init || !m_lower_init)
    {
        std::cout << "Please load both upper teeth and lower teeth!";
...
void Function::compute_paired_wires(int index, bool flag_lower, double distance)
{
    if (flag_lower)
...
void Function::compute_wire_shapes(int index, bool flag_lower, int geodesic_step, double width)
{
    if (flag_lower)
...
void Function::update_pair_cut_plane(int index, bool flag_lower,
    ...
    DataList& pad_shape_normals)
{
    ...
    if (!pad_shape.empty())
    {
        CGAL::Polygon_mesh_processing::compute_face_normals(pad_shape, boost::make_assoc_property_map(fnormals));

        for (Facet_iterator face = pad_shape.facets_begin(); face != pad_shape.facets_end(); ++face)
        {
            Halfedge_facet_circulator he = face->facet_begin();
            Vector face_normal = fnormals[face];
            do {
                Point p = he->vertex()->point();
                pad_shape_points.push_back((float)p.x());
                pad_shape_points.push_back((float)p.y());
                pad_shape_points.push_back((float)p.z());
                pad_shape_normals.push_back((float)face_normal.x());
                pad_shape_normals.push_back((float)face_normal.y());
                pad_shape_normals.push_back((float)face_normal.z());
            } while (++he != face->facet_begin());
        }
    }
}
